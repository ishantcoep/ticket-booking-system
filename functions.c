...
void repeated_user(struct login) {
	int c = 0;
	char str[120];
	char *tmp
	char a[10];
...
void ticketdisplay(struct movie) {
	...
	switch(a)
		CASE 1 :
			ticketbookingandpayment(seat, movie, details);
			break;
		CASE 2 :
...
void ticketprint(int a, int b) {
	...
	fclose(fp);
	printf("\n\n\n\t\t\t****************************************\n\t\t\tNAME OF THE MOVIE:%s \n\t\t\tNUMBER OF TICKETS:%d \n\t\t\tSEAT NUMBER:%s \n\t\t\tTIME:%s \n\t\t\tSCREEN NO.: %d\n\t\t\tTOTAL PRICE: %d\n\t\t\t****************************************\n", show[a].showname, b, seat.seat_num, show[a].time, show[a].screen_num, show[a].price); //printing ticket seat number gives problem
...
void ticketarray(int a, struct movie) {
	int i, j, d = 65;
	char c[2], str[3];
	FILE *fp;
	fp = fopen("show1.txt", "r");
...
void seatbooking(int a, int b) {
	...
	scanf("%c", f);
	FILE *fp;
	fp = fopen("show1.txt", "r");         //algorithm for writing to different files for diff movies
	for(i = 0; i < 5; i++) {
		for(j = 0; j < 20; j++) {
			fscanf(fp, "%s\t", show[a].seat_nums[i][j]);
		}
